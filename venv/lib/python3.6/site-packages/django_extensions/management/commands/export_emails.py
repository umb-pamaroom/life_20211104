...
def full_name(**kwargs):
    ...
    name = " ".join(n for n in [name, introduction] if n)
    if name:
        return name
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        order_by = getattr(settings, 'EXPORT_EMAILS_ORDER_BY', ['introduction', 'name', 'username', 'email'])
        fields = getattr(settings, 'EXPORT_EMAILS_FIELDS', ['introduction', 'name', 'username', 'email'])
    ...
    def linkedin(self, qs):
        ...
        for ent in qs:
            csvf.writerow([ent.get('name', ''), ent.get('introduction', ''), ent.get('email', '')])
    ...
    def vcard(self, qs):
        ...
        for ent in qs:
            ...
            if ent.get('introduction') and ent.get('name'):
                card.add('n').value = vobject.vcard.Name(ent['introduction'], ent['name'])
            else:
